...
//Thread count global var
int thread_count = 6;
//Creating global Image and Matrix variable
...
void* pth_convolute(void *ID){
    int row,pix,bit,span;
    span=(&srcImage)->bpp*(&srcImage)->bpp;
    int thread = (int) ID;	
    for (row=(thread) * ((&srcImage)->height / thread_count); row<(thread + 1) * ((&srcImage)->height / thread_count); row++){
...
int main(int argc,char** argv){
	...
	//Moved srcImage and destImage to be global vars
    Image bwImage;   
	...
	pthread_create(&handles[thread], NULL, &pth_convolute, (void*) thread);
	for(thread = 0; thread < thread_count; thread++)
